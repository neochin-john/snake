...
void MainScene::Update()
{
    ...
    std::cout << "preMoveStep: " << snake.Speed() * deltaFrame * fixedFrameTime << std::endl;
    std::cout << "speed: " << snake.Speed() << ", fixedFrameTime: "<< fixedFrameTime << ", frame: " << frame << ", lastSnakeMoveFrame: " << lastSnakeMoveFrame << std::endl;
    moveStep = moveStep == 0 ? (int)(snake.Speed() * deltaFrame * fixedFrameTime ) : moveStep;
...
void MainScene::Rende()
{
	...
	for (int y = 0; y < map.Rows(); y++)
	{
	    ...
	    DrawRectangleLines(x * blockSize, y * blockSize, blockSize, blockSize, DARKGRAY);
	}
    }

    // food
    DrawRectangle((int)food.x * blockSize, (int)food.y * blockSize, blockSize, blockSize, SKYBLUE);

    // snake
    auto body = snake.Body();
    for (auto iter = body.begin(); iter != body.end(); iter++)
    {
	DrawRectangle(iter->x * blockSize, iter->y * blockSize, blockSize, blockSize, PINK);
    }
    EndDrawing();
